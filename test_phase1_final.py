...
def test_phase1_core():
    ...
    try:
        ...
        fund_data = pd.DataFrame(
            {
                "date": pd.date_range("2023-01-01", periods=132, freq="D").strftime(
                    "%Y-%m-%d"
                ),
        ...
        index_data = pd.DataFrame(
            {
                "date": pd.date_range("2023-01-01", periods=24, freq="W").strftime(
                    "%Y-%m-%d"
                ),
        ...
        fund_dates = fund_pl.select(
            pl.col("date").str.strptime(pl.Date, format="%Y-%m-%d")
        ).to_series()
        index_dates = index_pl.select(
            pl.col("date").str.strptime(pl.Date, format="%Y-%m-%d")
        ).to_series()
        ...
        print(
            f"✅ Aligned shapes: Fund={fund_aligned.shape}, Index={index_aligned.shape}"
        )
    ...
    try:
        ...
        print(
            f"✅ Success envelope: {success_envelope.success}, data={success_envelope.data}"
        )

        # Error case
        error_envelope = ErrorEnvelope(False, errors=["Test error"])
        print(
            f"✅ Error envelope: {error_envelope.success}, errors={len(error_envelope.errors)}"
        )
