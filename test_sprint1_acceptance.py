...
def test_single_request_performance():
    ...
    assert first_response_time < 1.0, f"First request too slow: {first_response_time:.3f}s"
    assert second_response_time < 1.0, f"Second request too slow: {second_response_time:.3f}s"
    
    print(f"   ✅ Both requests under 1 second")
...
def test_concurrent_requests():
    ...
    def make_request(request_id: int) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "data_length": len(response.json().get("data", [])) if response.status_code == 200 else 0
            }
    ...
    if response_times:
        ...
        # Performance assertions
        assert len(successful_requests) >= len(results) * 0.95, "At least 95% requests should succeed"
...
def test_cache_behavior():
    ...
    print(f"   ✅ All responses identical (cache consistent)")
    ...
    if later_responses:
        ...
        assert avg_later_time < 0.5, f"Cached responses should be under 500ms: {avg_later_time:.3f}s"
...
def test_data_integrity():
    ...
    for i, series in enumerate(chart_data):
        ...
        assert len(series["x"]) == len(series["y"]), f"Series {i} x and y data must have same length"
...
def main():
    ...
    try:
        ...
        print(f"   • Single request performance: ✅ {first_time*1000:.1f}ms / {second_time*1000:.1f}ms")
        print(f"   • Concurrent handling: ✅ {len([r for r in concurrent_results if r['success']])}/{len(concurrent_results)} requests succeeded")
        print(f"   • Cache consistency: ✅ All responses identical")
        print(f"   • Data integrity: ✅ {len(data['data'])} series with valid structure")
        print()
