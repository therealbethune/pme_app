...
def test_analysis_integration():
    ...
    try:
        ...
        print("   Start the server with: python3 pme_calculator/backend/main_minimal.py")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'metrics' in analysis_results:
                ...
                if 'Analytics Data' in metrics and 'performance_timeline' in metrics['Analytics Data']:
                    timeline = metrics['Analytics Data']['performance_timeline']
                    if timeline:
                        for i, point in enumerate(timeline[:3]):  # Show first 3 points
                            print(f"   Point {i+1}: Date={point.get('date')}, TVPI={point.get('tvpi'):.3f}, IRR={point.get('irr'):.1f}%")
                        if len(timeline) > 3:
