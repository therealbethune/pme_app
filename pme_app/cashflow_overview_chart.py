...
def plot_cashflow_overview(
    ...
    if not isinstance(currency_symbol, str):
        raise TypeError("currency_symbol must be a string, not a DataFrame or other type.")
    ...
    # Main lines
    nav_line, = ax.step(df.index, df["nav"], label="NAV", color="#B0B0B0", linewidth=2.5, where="post")
    tv_line, = ax.step(df.index, df["total_value"], label="Total Value", color="#232323", linewidth=2.8, where="post")
    paidin_line, = ax.step(df.index, df["cum_paid_in"], label="Total Paid In", color="#e43b67", linewidth=2.2, where="post")
    distr_line, = ax.step(df.index, df["cum_distribs"], label="Total Distributions", color="#39cfff", linewidth=2.2, where="post")
    cf_line, = ax.step(df.index, df["cashflows"], label="Cash Flows", color="#A0A0A0", linewidth=1.1, where="post", alpha=0.8)
    ...
    ax.legend(handles, [h.get_label() for h in handles], loc="lower center", bbox_to_anchor=(0.5, -0.23), ncol=5, frameon=False, fontsize=12)
    ...
    def on_add(sel):
        ...
        sel.annotation.get_bbox_patch().set(fc="white", alpha=0.97, boxstyle="round,pad=0.5", lw=1.3, ec="#393939")
