...
# Add backend directory to Python path for pme_math imports
backend_dir = Path(__file__).parent.parent / "pme_calculator" / "backend"
...
def calculate_direct_alpha_pme(fund_df, index_returns, risk_free_rate):
    ...
    # Calculate PME as ratio of (1 + fund_irr) / (1 + index_irr)
    if not np.isnan(fund_irr) and not np.isnan(index_irr) and (1 + index_irr) != 0:
        pme_value = (1 + fund_irr) / (1 + index_irr)
    else:
        pme_value = np.nan
    pme_irr = fund_irr
...
def calculate_modified_pme(fund_df, index_returns, risk_free_rate):
    ...
    # Risk-adjusted benchmark return (index return is already the benchmark)
    index_irr = _calculate_annualized_return(index_returns)
    risk_adjusted_benchmark = index_irr if not np.isnan(index_irr) else np.nan
