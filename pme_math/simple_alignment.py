...
class DataAlignmentEngine:
    ...
    def align_fund_and_index(self, fund_data, index_data, 
        ...
        # Convert to Polars
        fund_pl = pl.from_pandas(fund_data) if isinstance(fund_data, pd.DataFrame) else fund_data
        index_pl = pl.from_pandas(index_data) if isinstance(index_data, pd.DataFrame) else index_data
