...
from pme_math.alignment_engine import DataAlignmentEngine, AlignmentStrategy
from pme_math.error_envelope import (
    ErrorEnvelope, ErrorDetail, ErrorSeverity, ErrorCategory,
    envelope_ok, envelope_fail, envelope_partial, 
    ErrorCollector, wrap_with_envelope,
    create_alignment_error, create_missing_data_warning
)
...
class TestDataAlignmentEngine:
    ...
    def sample_fund_data(self) -> pd.DataFrame:
        ...
        dates = [
            "2023-01-01", "2023-02-15", "2023-04-01", 
            "2023-06-15", "2023-09-01", "2023-12-01"
        ]
    ...
    def sample_index_data(self) -> pd.DataFrame:
        ...
        for i in range(90):  # 90 days of data
            ...
            if current_date.weekday() < 5:
                ...
                values.append(100 + i * 0.1 + (i % 7) * 0.5)  # Trending upward with noise
    ...
    def test_mismatch_alignment_fix(self, mismatched_data):
        ...
        expected_days = (pd.to_datetime(fund_df["date"].max()) - 
                        pd.to_datetime(fund_df["date"].min())).days + 1
        # Filter to business days
        business_days = sum(1 for i in range(expected_days) 
    ...
    def test_missing_value_strategies(self, sample_fund_data, sample_index_data):
        ...
        for strategy in strategies:
            ...
            if strategy != "drop":
                ...
                fund_nulls = fund_aligned.select(pl.col("fund_value").is_null().sum()).item()
                index_nulls = index_aligned.select(pl.col("index_value").is_null().sum()).item()
    ...
    def test_realistic_pme_scenario(self):
        ...
        try:
            ...
            collector.add_error(
                ...
                context={"fund_shape": fund_data.shape, "index_shape": index_data.shape}
            )
...
if __name__ == "__main__":
    ...
    index_data = pd.DataFrame({
        "date": ["2023-01-01", "2023-03-01", "2023-06-01"], 
        "value": [100, 105, 110]
    })
