...
class IntelligentDataProcessor:
    ...
    def _initialize_column_patterns(self) -> Dict[DataType, List[str]]:
        ...
        return {
            DataType.DATE: [
                r'.*date.*', r'.*time.*', r'.*period.*', r'.*month.*', r'.*year.*',
                r'.*day.*', r'.*dt.*', r'.*timestamp.*', r'.*when.*'
            ],
            DataType.CASH_FLOW: [
                r'.*cash.*flow.*', r'.*cashflow.*', r'.*cf.*', r'.*flow.*',
                r'.*net.*cash.*', r'.*cash.*'
            ],
            DataType.CONTRIBUTION: [
                r'.*contribution.*', r'.*contrib.*', r'.*capital.*call.*', r'.*call.*',
                r'.*paid.*in.*', r'.*investment.*', r'.*funding.*', r'.*drawdown.*',
                r'.*commitment.*', r'.*inflow.*'
            ],
            DataType.DISTRIBUTION: [
                r'.*distribution.*', r'.*distrib.*', r'.*payout.*', r'.*dividend.*',
                r'.*return.*capital.*', r'.*proceeds.*', r'.*realization.*',
                r'.*outflow.*', r'.*cash.*out.*'
            ],
            DataType.NAV: [
                r'.*nav.*', r'.*net.*asset.*value.*', r'.*valuation.*', r'.*value.*',
                r'.*market.*value.*', r'.*fair.*value.*', r'.*book.*value.*',
                r'.*ending.*value.*', r'.*balance.*'
            ],
            DataType.INDEX_VALUE: [
                r'.*index.*', r'.*benchmark.*', r'.*market.*', r'.*s&p.*', r'.*sp.*',
                r'.*russell.*', r'.*msci.*', r'.*level.*', r'.*price.*index.*'
            ],
            DataType.PRICE: [
                r'.*price.*', r'.*level.*', r'.*quote.*', r'.*rate.*'
            ],
            DataType.RETURN: [
                r'.*return.*', r'.*yield.*', r'.*performance.*', r'.*gain.*',
                r'.*growth.*', r'.*change.*'
            ]
        }
    ...
    def _analyze_sample_values(self, sample_values: List[Any]) -> Tuple[DataType, float]:
        ...
        if len(numeric_values) / len(clean_values) > 0.8:
            ...
            if cash_flow_patterns['is_likely_cash_flow'] and not cash_flow_patterns['specific_type']:
    ...
    def _detect_cash_flow_patterns(self, numeric_values: List[float]) -> Dict[str, Any]:
        ...
        is_likely_cash_flow = (
            has_large_amounts and (has_negative or has_positive) and
            'percentage_like' not in patterns
        )
    ...
    def process_dataset(self, 
                       ...
                       dataset_name: str = "dataset") -> Tuple[pd.DataFrame, DatasetMetadata]:
        """
    ...
    def _apply_transformations(self, df: pd.DataFrame, mappings: List[ColumnMapping]) -> pd.DataFrame:
        ...
        for mapping in mappings:
            ...
            elif mapping.data_type in [DataType.CASH_FLOW, DataType.CONTRIBUTION, 
                                     DataType.DISTRIBUTION, DataType.NAV, DataType.CURRENCY]:
    ...
    def _calculate_metadata(self, name: str, df: pd.DataFrame, mappings: List[ColumnMapping]) -> DatasetMetadata:
    ...
    def detect_data_issues(self, df: pd.DataFrame, mappings: List[ColumnMapping], dataset_name: str = "dataset") -> List[DataIssue]:
    ...
    def _detect_date_issues(self, df: pd.DataFrame, mappings: List[ColumnMapping]) -> List[DataIssue]:
                ...
                if col_name in df.columns:
                    ...
                    if invalid_dates:
                        issues.append(DataIssue(
                            ...
                            fix_parameters={"column": col_name, "format": "standard_date"}
                        ))
        
        return issues
    
    def _detect_missing_value_issues(self, df: pd.DataFrame, mappings: List[ColumnMapping]) -> List[DataIssue]:
        """Detect critical missing values"""
        issues = []
        
        critical_types = [DataType.DATE, DataType.CASH_FLOW, DataType.CONTRIBUTION, DataType.DISTRIBUTION]
                ...
                if col_name in df.columns:
                    ...
                    if missing_pct > 10:  # More than 10% missing
                        ...
                        issues.append(DataIssue(
                            ...
                            fix_parameters={"column": col_name, "method": "interpolate", "type": mapping.data_type.value}
                        ))
        
        return issues
    
    def _detect_cash_flow_sign_issues(self, df: pd.DataFrame, mappings: List[ColumnMapping]) -> List[DataIssue]:
        """Detect cash flow sign inconsistencies"""
        issues = []
        
        cash_flow_types = [DataType.CONTRIBUTION, DataType.DISTRIBUTION, DataType.CASH_FLOW]
                        ...
                        if mapping.data_type == DataType.CONTRIBUTION:
                            ...
                            if positive_count > len(series) * 0.8:  # More than 80% positive
                                issues.append(DataIssue(
                                    ...
                                    fix_parameters={"column": col_name, "operation": "negate_positive"}
                                ))
                            ...
                            if negative_count > len(series) * 0.8:  # More than 80% negative
                                issues.append(DataIssue(
                                    ...
                                    fix_parameters={"column": col_name, "operation": "negate_negative"}
                                ))
    ...
    def _detect_naming_issues(self, df: pd.DataFrame) -> List[DataIssue]:
        ...
        if problematic_columns:
            issues.append(DataIssue(
                ...
                fix_parameters={"columns": problematic_columns, "operation": "standardize_names"}
            ))
        
        return issues
    
    def _detect_numeric_issues(self, df: pd.DataFrame, mappings: List[ColumnMapping]) -> List[DataIssue]:
        """Detect invalid numeric values"""
        issues = []
        
        numeric_types = [DataType.CASH_FLOW, DataType.CONTRIBUTION, DataType.DISTRIBUTION, 
                        DataType.NAV, DataType.PRICE, DataType.RETURN]
                ...
                if col_name in df.columns:
                    ...
                    if string_numeric_rows:
                        issues.append(DataIssue(
                            ...
                            fix_parameters={"column": col_name, "operation": "string_to_numeric"}
                        ))
    ...
    def apply_data_fix(self, df: pd.DataFrame, issue: DataIssue) -> pd.DataFrame:
        ...
        try:
            if issue.fix_parameters.get("operation") == "negate_positive":
                ...
                col = issue.fix_parameters["column"]
                if issue.fix_parameters.get("type") in ["cash_flow", "contribution", "distribution"]:
    ...
    def create_optimal_structure(self, 
                               ...
                               column_classifications: Optional[Dict[str, str]] = None) -> OptimalDataStructure:
        """
            ...
            try:
                ...
                for mapping in dataset_metadata.column_mappings:
                    if mapping.data_type == DataType.OTHER_CASH_FLOW:
                        ...
                        unclassified_columns.append(UnclassifiedColumn(
                ...
                logger.info(f"Processed dataset '{name}': {dataset_metadata.rows} rows, "
        ...
        for name, df in processed_datasets.items():
            ...
            fund_indicators = {DataType.CONTRIBUTION, DataType.DISTRIBUTION, DataType.NAV, DataType.CASH_FLOW}
            ...
            if fund_indicators.intersection(column_types):
                if fund_data is None or dataset_meta.data_quality_score > metadata[list(processed_datasets.keys())[0]].data_quality_score:
                    fund_data = df
        ...
        if unclassified_columns:
            ...
            warnings.append(f"Found {len(unclassified_columns)} cash flow columns that need classification")
            suggestions.append("Please classify the ambiguous cash flow columns to proceed with analysis")
        else:
            # Validate calculation readiness
            calculation_ready = self._validate_calculation_readiness(fund_data, index_data, warnings, suggestions)
    ...
    def _validate_calculation_readiness(self, 
                                      fund_data: pd.DataFrame, 
                                      index_data: Optional[pd.DataFrame],
                                      warnings: List[str],
        ...
        has_contribution_distribution = 'contribution' in fund_data.columns and 'distribution' in fund_data.columns
        
        if not (has_cashflow or has_contribution_distribution):
            warnings.append("Fund data missing cash flow information")
            suggestions.append("Ensure you have either 'cashflow' column or both 'contribution' and 'distribution' columns")
            ready = False
        ...
        if index_data is not None:
            ...
            index_value_cols = [col for col in index_data.columns if col in ['index_value', 'price', 'level']]
...
def process_multiple_datasets(datasets: Dict[str, Any], primary_dataset: str = None) -> OptimalDataStructure:
    """
