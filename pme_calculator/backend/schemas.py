...
class AnalysisRequest(BaseModel):
    ...
    calculation_methods: List[str] = Field(default=["kaplan_schoar", "pme_plus", "direct_alpha"])
...
class DataValidator:
    ...
    @staticmethod
    def intelligent_column_mapping(columns: List[str], data_type: str = "fund") -> List[ColumnMapping]:
        """Intelligent column mapping with confidence scoring."""
        mappings = []
        
        if data_type == "fund":
            # Date mapping
            date_keywords = ['date', 'period', 'time', 'month', 'quarter', 'year', 'timestamp']
            cashflow_keywords = ['cashflow', 'cash_flow', 'cash flow', 'cf', 'net_cash', 'amount', 'contribution', 'distribution']
            ...
            for col in columns:
                ...
                # Date column mapping
                date_confidence = max([0.9 if keyword in col_lower else 0 for keyword in date_keywords])
                ...
                # Cashflow mapping
                cf_confidence = max([0.9 if keyword in col_lower else 0 for keyword in cashflow_keywords])
                ...
                # NAV mapping
                nav_confidence = max([0.9 if keyword in col_lower else 0 for keyword in nav_keywords])
            ...
            price_keywords = ['price', 'level', 'index', 'value', 'close', 'closing', 'adjusted']
            ...
            for col in columns:
                ...
                # Price mapping
                price_confidence = max([0.9 if keyword in col_lower else 0 for keyword in price_keywords])
