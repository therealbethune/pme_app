...
def run_metrics(self, fund_path: str, index_path: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        self.update_state(
            state='PROGRESS',
            meta={'current': 0, 'total': 100, 'status': 'Initializing analysis engine...'}
        )
        ...
        if index_path:
            self.update_state(
                state='PROGRESS',
                meta={'current': 40, 'total': 100, 'status': 'Loading benchmark data...'}
            )
        ...
        logger.info(f"Background PME analysis task {self.request.id} completed successfully")
        ...
        logger.error(f"Background PME analysis task {self.request.id} failed: {exc}", exc_info=True)
...
@celery.task(bind=True)
def generate_pdf_report(self, analysis_results: Dict[str, Any], report_config: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        self.update_state(
            state='PROGRESS',
            meta={'current': 0, 'total': 100, 'status': 'Initializing PDF generator...'}
        )
        ...
        self.update_state(
            state='PROGRESS',
            meta={'current': 50, 'total': 100, 'status': 'Generating charts and tables...'}
        )
        ...
        logger.error(f"PDF report generation task {self.request.id} failed: {exc}", exc_info=True)
...
POPULAR_FUNDS = ["FUND_A", "FUND_B", "FUND_C"]  # TODO: Make this dynamic based on usage analytics
...
def warm_cache(self) -> Dict[str, Any]:
    ...
    try:
        ...
        self.update_state(
            state='PROGRESS',
            meta={'current': 0, 'total': 100, 'status': 'Refreshing materialized views...'}
        )
        ...
        self.update_state(
            state='PROGRESS',
            meta={'current': 20, 'total': 100, 'status': 'Warming cache for popular funds...'}
        )
        ...
        result = {
            ...
            'success_rate': len(warmed_funds) / len(POPULAR_FUNDS) if POPULAR_FUNDS else 0,
            'task_id': self.request.id
        }
        
        logger.info(f"Cache warming task {self.request.id} completed. Warmed {len(warmed_funds)}/{len(POPULAR_FUNDS)} funds")
        
        return result
        
    except Exception as exc:
        logger.error(f"Cache warming task {self.request.id} failed: {exc}", exc_info=True)
