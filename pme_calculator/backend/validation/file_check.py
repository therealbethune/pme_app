...
def detect_column_mappings(df: pd.DataFrame, file_type: str = 'fund') -> Dict[str, str]:
    ...
    if file_type == 'fund':
        ...
        date_patterns = ['date', 'period', 'as_of', 'valuation_date', 'quarter', 'month']
        cashflow_patterns = ['cashflow', 'cash_flow', 'net_cashflow', 'cf', 'cash_flows', 
                           'contributions_distributions', 'net_cf', 'cash_flow_amount']
        nav_patterns = ['nav', 'net_asset_value', 'value', 'valuation', 'fund_value', 'portfolio_value']
        contrib_patterns = ['contributions', 'contribution', 'calls', 'capital_calls', 'drawdowns']
        ...
        for col in columns:
            ...
            # Date column
            if not mappings.get('date') and any(pattern in col_clean for pattern in date_patterns):
                mappings['date'] = df.columns[columns.index(col)]
                
            # Cashflow column
            elif not mappings.get('cashflow') and any(pattern in col_clean for pattern in cashflow_patterns):
                mappings['cashflow'] = df.columns[columns.index(col)]
                
            # NAV column
            elif not mappings.get('nav') and any(pattern in col_clean for pattern in nav_patterns):
                mappings['nav'] = df.columns[columns.index(col)]
                
            # Contributions
            elif not mappings.get('contributions') and any(pattern in col_clean for pattern in contrib_patterns):
                mappings['contributions'] = df.columns[columns.index(col)]
                
            # Distributions
            elif not mappings.get('distributions') and any(pattern in col_clean for pattern in distrib_patterns):
        ...
        price_patterns = ['price', 'level', 'index_level', 'value', 'close', 'closing_price', 'index']
        ...
        for col in columns:
            ...
            # Price column
            elif not mappings.get('price') and any(pattern in col_clean for pattern in price_patterns):
                mappings['price'] = df.columns[columns.index(col)]
                
            # Returns column
            elif not mappings.get('returns') and any(pattern in col_clean for pattern in return_patterns):
                mappings['returns'] = df.columns[columns.index(col)]
    
    return mappings


def validate_csv_structure(file_path: Union[str, Path]) -> Tuple[pd.DataFrame, List[str]]:
    """
...
def validate_fund_file(file_path: Union[str, Path]) -> List[str]:
    ...
    if missing_required:
        errors.append(f"Could not identify required columns: {', '.join(missing_required)}")
    ...
    try:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                errors.append(f"Row {idx + 2}: {str(e)}")  # +2 for header and 0-indexing
...
def create_upload_metadata(file_path: Union[str, Path], df: pd.DataFrame = None) -> UploadMeta:
...
def validate_file_comprehensive(file_path: Union[str, Path], file_type: str = 'fund') -> ValidationResult:
    """
