...
def calculate_pme_metrics_sync(request: AnalysisRequest, fund_file_data: dict) -> dict[str, Any]:
    ...
    metrics = {
        ...
        "Analysis Date": datetime.now(UTC).strftime("%Y-%m-%d %H:%M:%S UTC"),
    }
...
async def cache_statistics():
    ...
    return {
        ...
        "oldest_entry": (
            min([entry["timestamp"] for entry in analysis_cache.values()])
            if analysis_cache
            else None
        ),
        "newest_entry": (
            max([entry["timestamp"] for entry in analysis_cache.values()])
            if analysis_cache
            else None
        ),
    }
...
async def clear_cache():
    ...
    logger.info("Analysis cache cleared", extra={"cleared_entries": cleared_count})
...
async def process_datasets(
    ...
    try:
        ...
        logger.info(
            f"Processed {len(datasets)} datasets successfully",
            extra={
...
async def upload_fund_data(
    ...
    try:
        ...
        if result["success"]:
            ...
            os.unlink(temp_path)
            raise HTTPException(
                status_code=400, detail=result.get("error", "Data validation failed")
            )
...
async def upload_benchmark_data(
    ...
    try:
        ...
        if result["success"]:
            ...
            raise HTTPException(
                status_code=400,
                detail=result.get("error", "Benchmark validation failed"),
            )
...
async def run_comprehensive_analysis(
    ...
    try:
        ...
        logger.info(
            ...
            extra={"request_id": request_id, "processing_time_ms": processing_time},
        )
...
def _run_monte_carlo_simulation_sync() -> dict[str, Any]:
    ...
    return {
        "iterations": 1000,
        "confidence_intervals": {"irr_95": [0.12, 0.25], "tvpi_95": [1.8, 2.9]},
        "percentiles": {
            "p5": 0.08,
            "p25": 0.15,
            "p50": 0.185,
