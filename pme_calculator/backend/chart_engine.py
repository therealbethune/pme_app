...
class ChartEngine:
    ...
    def create_pme_dashboard(self, fund_data: pd.DataFrame, benchmark_data: pd.DataFrame, 
        ...
        try:
            ...
            charts['risk_return'] = self._create_risk_return_chart(fund_data, benchmark_data)
            ...
            charts['distributions_timeline'] = self._create_distributions_timeline(fund_data)
    ...
    def _create_performance_chart(self, fund_data: pd.DataFrame, 
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=(
                'Cumulative Performance', 'Monthly Returns',
                'PME Comparison', 'NAV vs Benchmark'
            ),
            specs=[
        ...
        fig.add_trace(
            go.Bar(
                ...
                y=pme_values,
                name='PME Metrics',
                marker_color=[self.color_palette['pme'], self.color_palette['distribution'], 
        ...
        if len(cash_flows) > 0:
            fig.add_trace(
                go.Scatter(
                    ...
                    name='Cash Flows',
                    marker=dict(
                        size=abs(cash_flows['cashflow']) / cash_flows['cashflow'].abs().max() * 20 + 5,
                        color=np.where(cash_flows['cashflow'] > 0, 
    ...
    def _create_risk_return_chart(self, fund_data: pd.DataFrame, 
        ...
        x_line = np.linspace(0, max(fund_volatility * 100, bench_volatility * 100) * 1.2, 100)
    ...
    def _create_rolling_performance_chart(self, fund_data: pd.DataFrame, 
                                        benchmark_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        fund_df['rolling_volatility'] = fund_df['nav'].pct_change().rolling(window=window).std()
    ...
    def export_chart_data(self, charts: Dict[str, Any], format: str = 'json') -> Dict[str, Any]:
