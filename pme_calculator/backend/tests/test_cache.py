...
from cache import CacheManager, cache, cached, get_cache, set_cache, delete_cache, cache_exists, init_cache
...
class TestCacheManager:
    ...
    async def test_initialize_with_redis_success(self):
        ...
        with patch('cache.REDIS_AVAILABLE', True), \
             patch('cache.aioredis') as mock_aioredis:
    ...
    async def test_set_success_json(self):
        ...
        assert result is True
        mock_redis.set.assert_called_once_with("test_key", json.dumps(test_data, default=str), ex=300)
    ...
    def test_generate_key_simple(self):
        ...
        key = cache_manager._generate_key("test_func", "arg1", "arg2", param1="value1", param2="value2")
