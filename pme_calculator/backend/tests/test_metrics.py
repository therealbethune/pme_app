    ...
    fund_data = pd.DataFrame(
        {
            ...
            "Cash_Flow": [
                ...
                200,
                800,
            ],  # Initial investment, additional, no flow, partial return, final return
    ...
    index_data = pd.DataFrame(
        {
            ...
            "Index_Value": [
                ...
                168,
                200,
            ],  # Index progression matching fund performance
        }
    )

    return {"fund": fund_data, "index": index_data}
...
def test_ks_pme_underperformance(sample_cashflow_data):
    ...
    assert (
        ks_result < 1.0
    ), f"Expected KS PME < 1.0 for underperformance, got {ks_result}"
...
def test_xirr_wrapper_known_figure():
    ...
    # Expected IRR = 10% = 0.1
    cashflow_dict = {"2020-01-01": -1000.0, "2021-01-01": 1100.0}
    ...
    assert (
        abs(calculated_irr - expected_irr) < 1e-3
    ), f"Expected IRR ≈ {expected_irr}, got {calculated_irr}"
...
def test_xirr_wrapper_complex_cashflows():
    ...
    assert not np.isnan(
        calculated_irr
    ), "XIRR calculation should not return NaN for valid cash flows"
    assert (
        -1.0 < calculated_irr < 10.0
    ), f"IRR should be reasonable, got {calculated_irr}"
...
def test_additional_metrics_functions():
    ...
    assert (
        abs(alpha - 0.045454545454545456) < 1e-10
    ), f"Direct alpha calculation incorrect: {alpha}"
