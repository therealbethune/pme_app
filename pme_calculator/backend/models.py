...
class Fund(Base):
    ...
    # Relationships
    cash_flows: Mapped[List["CashFlow"]] = relationship("CashFlow", back_populates="fund", cascade="all, delete-orphan")
    nav_entries: Mapped[List["NAV"]] = relationship("NAV", back_populates="fund", cascade="all, delete-orphan")
    portfolio_funds: Mapped[List["PortfolioFund"]] = relationship("PortfolioFund", back_populates="fund")
    
    __table_args__ = (
        Index('idx_fund_name_vintage', 'name', 'vintage_year'),
    )
...
class Portfolio(Base):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    benchmark_symbol: Mapped[str] = mapped_column(String(20), default="^GSPC")  # S&P 500
    ...
    # Relationships
    portfolio_funds: Mapped[List["PortfolioFund"]] = relationship("PortfolioFund", back_populates="portfolio", cascade="all, delete-orphan")
...
class PortfolioFund(Base):
    ...
    # Relationships
    portfolio: Mapped["Portfolio"] = relationship("Portfolio", back_populates="portfolio_funds")
...
class CashFlow(Base):
    ...
    amount: Mapped[float] = mapped_column(Float, nullable=False)  # Positive = contribution, Negative = distribution
    cash_flow_type: Mapped[str] = mapped_column(String(50), nullable=False)  # contribution, distribution, management_fee
...
class NAV(Base):
    ...
    __table_args__ = (
        Index('idx_nav_fund_date', 'fund_id', 'date'),
    )
...
class BenchmarkPrice(Base):
    ...
    __table_args__ = (
        Index('idx_benchmark_symbol_date', 'symbol', 'date'),
    )
...
class UploadHistory(Base):
    ...
    file_type: Mapped[str] = mapped_column(String(20), nullable=False)  # fund, index, nav
    upload_date: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    rows_processed: Mapped[int] = mapped_column(Integer, default=0)
    status: Mapped[str] = mapped_column(String(20), default="success")  # success, error, partial
    error_message: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    fund_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("funds.id"), nullable=True)
    
    __table_args__ = (
        Index('idx_upload_date_type', 'upload_date', 'file_type'),
    ) 