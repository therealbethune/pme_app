...
try:
    ...
    from database import (
        init_db, get_session, UploadFileMeta, 
        create_upload_record, get_upload_by_id, 
        get_uploads_by_user, update_upload_record, delete_upload_record
    )
...
async def upload_fund_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        columns_count = len(engine.fund_data.columns) if engine.fund_data is not None else 0
...
async def upload_index_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        columns_count = len(engine.index_data.columns) if engine.index_data is not None else 0
    ...
    return [
        {
            ...
            "funds": [
                {"id": 1, "name": "TechVenture Fund I", "fund_name": "TechVenture Fund I", "vintage_year": 2018},
                {"id": 2, "name": "Growth Equity Fund", "fund_name": "Growth Equity Fund", "vintage_year": 2019}
            ]
        },
        {
            "id": 2,
            ...
            "funds": [
                {"id": 3, "name": "Real Estate Fund", "fund_name": "Real Estate Fund", "vintage_year": 2017},
                {"id": 4, "name": "Infrastructure Fund", "fund_name": "Infrastructure Fund", "vintage_year": 2020}
            ]
        }
    ]
...
async def get_portfolio_analytics(portfolio_id: int):
    ...
    analytics = await run_in_threadpool(_calculate_portfolio_analytics_sync, portfolio_id)
...
async def run_analysis_sync(fund_file_id: str, index_file_id: Optional[str] = None):
    ...
    if fund_file_id not in uploaded_files or uploaded_files[fund_file_id]["type"] != "fund":
    ...
    if index_file_id:
        if index_file_id not in uploaded_files or uploaded_files[index_file_id]["type"] != "index":
            raise HTTPException(404, f"Index file with ID '{index_file_id}' not found.")
        index_path = uploaded_files[index_file_id]["path"]
    
    try:
        logger.info(f"Starting synchronous PME analysis", extra={
        ...
        analysis_result = await run_in_threadpool(_run_pme_analysis_sync, fund_path, index_path)
        
        logger.info(f"Synchronous PME analysis completed", extra={
            'fund_file_id': fund_file_id,
        ...
        logger.error(f"Failed to run synchronous analysis for fund {fund_file_id}: {e}", exc_info=True)
...
async def run_analysis(fund_file_id: str, index_file_id: Optional[str] = None):
    ...
    try:
        logger.info(f"Starting background PME analysis task", extra={
            'fund_file_id': fund_file_id,
            'index_file_id': index_file_id,
            'has_benchmark': index_path is not None
        })
        ...
        logger.info(f"PME analysis task queued", extra={
            'task_id': task.id,
            'fund_file_id': fund_file_id
        })
        ...
        logger.error(f"Failed to start analysis task for fund {fund_file_id}: {e}", exc_info=True)
...
async def get_irr_pme_chart(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        fund_files = [f for f in uploaded_files.values() if f.get('type') == 'fund' and f.get('analysis_results')]
                    ...
                    "data": [
                        {
                            ...
                            "x": dates,
                            "y": tvpi_values,
                            "line": {"shape": "spline", "smoothing": 0.3, "color": "#00ff88", "width": 4},
                        ...
                        },
                        {
                            ...
                            "x": dates,
                            "y": dpi_values,
                            "line": {"shape": "spline", "smoothing": 0.3, "color": "#0066cc", "width": 3},
                        ...
                        },
                        {
                            ...
                            "x": dates,
                            "y": rvpi_values,
                            "line": {"shape": "spline", "smoothing": 0.3, "color": "#ff6b6b", "width": 3},
    ...
    dates = ['2020-Q1', '2020-Q2', '2020-Q3', '2020-Q4', '2021-Q1', '2021-Q2', '2021-Q3', '2021-Q4']
...
async def get_twr_vs_index_chart():
    ...
    try:
        ...
        if fund_files:
            ...
            if timeline_data:
                dates = [item['date'] for item in timeline_data]
                fund_values = [item.get('fund_performance', 1.0) for item in timeline_data]
                index_values = [item.get('index_performance', 1.0) for item in timeline_data]
                    ...
                    "data": [
                        {
                            ...
                            "x": dates,
                            "y": fund_values,
                                                    "line": {"shape": "spline", "smoothing": 1.3, "color": "#00ff88", "width": 3}
                    },
                    {
                        "type": "scatter",
                        "mode": "lines", 
                        ...
                        "x": dates,
                        "y": index_values,
                        "line": {"shape": "spline", "smoothing": 1.3, "color": "#0066cc", "dash": "dot", "width": 2}
                    }
                ],
                "layout": {
                    "yaxis": {
    ...
    dates = ['2020-01', '2020-04', '2020-07', '2020-10', '2021-01', '2021-04', '2021-07', '2021-10']
    ...
    try:
        ...
        if fund_files:
            ...
            if files_with_analysis:
                ...
                latest_file = max(files_with_analysis, key=lambda x: x.get('last_analysis_time', ''))
...
def get_port_config() -> int:
    ...
    try:
        ...
        logger.warning(f"Invalid port value in environment: {os.getenv('PORT')}, using default 8000")
        return 8000

def start_server_with_fallback(preferred_port: int = 8000, dry_run: bool = False) -> Dict[str, Any]:
    ...
    if not dry_run:
        logger.info(f"Starting Fund Analysis Tool Minimal FastAPI Backend")
...
def create_app_with_config(port: Optional[int] = None) -> tuple[FastAPI, Dict[str, Any]]:
