...
class ReportingService:
    ...
    def _render_pdf_template(self, portfolio: Portfolio, analytics: Dict, charts: Dict) -> str:
    ...
    async def _write_summary_sheet(self, writer: pd.ExcelWriter, portfolio_id: int):
        ...
        try:
            ...
            for key, value in risk_metrics.items():
                summary_data.append(['Risk Metrics', key.replace('portfolio_', '').replace('_', ' ').title(), value])
    ...
    async def _write_fund_sheet(self, writer: pd.ExcelWriter, fund_id: int, fund_name: str):
        ...
        try:
            ...
            if analytics.get('correlation_matrix'):
                corr_df = pd.DataFrame(
                    ...
                    columns=[f'Fund_{i+1}' for i in range(len(analytics['correlation_matrix']))],
                    index=[f'Fund_{i+1}' for i in range(len(analytics['correlation_matrix']))]
                )
            ...
            if analytics.get('optimal_weights', {}).get('optimization_success'):
                ...
                for i, (current, optimal) in enumerate(zip(current_weights, optimal_weights)):
                    weights_data.append({
                        'Fund': f'Fund_{i+1}',
