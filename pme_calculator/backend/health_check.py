    ...
    try:
        ...
        try:
            ...
            if result['success']:
                ...
                errors.append(f"Fund data loading failed: {result.get('error', 'Unknown error')}")
                logger.error(f"   ❌ Fund data loading failed: {result.get('error', 'Unknown error')}")
            ...
            if engine.fund_data is not None:
                ...
                if 'Fund IRR' in metrics:
                    ...
                    logger.error("   ❌ Metrics calculation returned incomplete results")
...
def test_fastapi_imports():
    ...
    try:
        ...
        try:
            from simple_analysis import router as simple_router
