...
try:
    ...
    try:
        ...
        def _direct_alpha(fund_irr, idx_irr):
            return 0.0
...
try:
    ...
    def ks_pme(fund_cf: np.ndarray, idx_values: np.ndarray) -> float:
        ...
        try:
            ...
            pv_contrib = (
                np.sum(contribs * (index_end / idx_contribs))
                if len(contribs) > 0 and np.all(idx_contribs > 0)
                else 0.0
            )
            pv_distrib = (
                np.sum(distribs * (index_end / idx_distribs))
                if len(distribs) > 0 and np.all(idx_distribs > 0)
                else 0.0
            )
...
def _pme_plus(cf, nav, idx, dates):
    ...
    try:
        ...
        lambda_val = 1.0 + (
            total_value / abs(np.sum(cf[cf < 0])) if np.sum(cf[cf < 0]) != 0 else 0
        )
...
class PMEAnalysisEngine:
    ...
    def load_fund_data(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Fund data loaded: {len(self.fund_data)} rows, {len(self.fund_data.columns)} columns"
            )
    ...
    def load_index_data(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Index data loaded: {len(self.index_data)} rows, {len(self.index_data.columns)} columns"
            )
    ...
    def calculate_pme_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            fund_dates = self.fund_data.get(
                "date", self.fund_data.get("Date", self.fund_data.iloc[:, 0])
            )
            fund_cashflows = self.fund_data.get(
                "cashflow", self.fund_data.get("Cashflow", self.fund_data.iloc[:, 1])
            )
            fund_navs = self.fund_data.get(
                "nav",
                self.fund_data.get(
                    "NAV",
                    (
                        self.fund_data.iloc[:, 2]
                        if len(self.fund_data.columns) > 2
                        else pd.Series([0] * len(self.fund_data))
                    ),
                ),
            )
            ...
            # Calculate multiples
            tvpi = (
                (total_distributions + final_nav) / total_contributions
                if total_contributions > 0
                else 0
            )
            dpi = (
                total_distributions / total_contributions
                if total_contributions > 0
                else 0
            )
            ...
            if self.index_data is not None:
                try:
                    ...
                    index_dates = self.index_data.get(
                        "date", self.index_data.get("Date", self.index_data.iloc[:, 0])
                    )
                    index_prices = self.index_data.get(
                        "price",
                        self.index_data.get("Price", self.index_data.iloc[:, 1]),
                    )
                    ...
                    index_df = (
                        pd.DataFrame({"date": index_dates, "price": index_prices})
                        .set_index("date")
                        .sort_index()
                    )
                    ...
                    if start_date <= end_date:
                        ...
                        common_dates = fund_aligned.index.intersection(
                            index_aligned.index
                        )
                        if len(common_dates) > 0:
                            # Use common dates for calculations
                            fund_cf_aligned = fund_aligned.loc[
                                common_dates, "cashflow"
                            ].values
                            index_prices_aligned = index_aligned.loc[
                                common_dates, "price"
                            ].values

                            logger.info(
                                f"Aligned data: {len(fund_cf_aligned)} common points"
                            )
                            ...
                            try:
                                index_returns = (
                                    index_prices_aligned[1:] / index_prices_aligned[:-1]
                                    - 1
                                )
                                index_cf_dict = dict(
                            ...
                            try:
                                ln_pme_irr, _ = _ln_pme(
                                    fund_cf_aligned, index_prices_aligned, common_dates
                                )
                                ln_pme_irr = safe_float(ln_pme_irr, 0.0)
                            except Exception as e:
                                logger.warning(
                                    f"Could not calculate Long-Nickels PME: {e}"
                                )
                                ln_pme_irr = 0.0
                        else:
                            logger.warning(
                                "No overlapping dates between fund and index data"
                            )
                    else:
                        logger.warning(
                            "No overlapping date range between fund and index data"
                        )
    ...
    def get_analysis_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": pme_metrics,
            "summary": {
                "kaplan_schoar_interpretation": (
                    "Above 1.0 indicates outperformance"
                    if pme_metrics.get("kaplan_schoar_pme", 1.0) > 1.0
                    else "Below 1.0 indicates underperformance"
                ),
                "alpha_interpretation": (
                    "Positive alpha indicates excess returns"
                    if pme_metrics.get("direct_alpha", 0.0) > 0
                    else "Negative alpha indicates underperformance"
                ),
            },
        }
