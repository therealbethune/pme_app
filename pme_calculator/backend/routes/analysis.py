...
import pandas as pd
import logging
from data_processor import IntelligentDataProcessor, OptimalDataStructure
import json
...
async def process_datasets(request: Request):
    ...
    try:
        ...
        for key, file in form.items():
            if hasattr(file, 'filename') and file.filename:
                try:
                    ...
                    if any(col.lower() in ['cashflow', 'contribution', 'distribution', 'nav'] 
        ...
        return {
            ...
            "fund_data_preview": optimal_structure.fund_data.head(5).to_dict(orient='records') if not optimal_structure.fund_data.empty else [],
            "index_data_preview": optimal_structure.index_data.head(5).to_dict(orient='records') if optimal_structure.index_data is not None else None
        }
...
async def suggest_optimal_structure(request: Request):
    ...
    try:
        ...
        if 'date' not in detected_type_names:
            suggestions["missing_requirements"].append("Date column is required for all calculations")
        
        if suggestions["detected_structure"]["type"] == "fund_data":
            if 'nav' not in detected_type_names:
                suggestions["missing_requirements"].append("NAV (Net Asset Value) column is required")
            
            has_cashflow = 'cashflow' in detected_type_names
            has_contrib_distrib = 'contribution' in detected_type_names and 'distribution' in detected_type_names
        ...
        if not suggestions["missing_requirements"]:
            suggestions["recommendations"].append("✅ All required columns detected - ready for PME calculations")
        else:
            suggestions["recommendations"].append("⚠️ Missing required columns - please review data structure")
        ...
        suggestions["recommendations"].append("🔄 Use the 'Process Datasets' endpoint to automatically optimize your data structure")
